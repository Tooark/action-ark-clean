...
export function buildPlan(c, versions, now = new Date()) {
    const decisions = [];
...
export function protectOciRelations(plan, evidence, c) {
    if (!c.protectMultiArch && !c.protectReferrers)
        return plan;
    ...
    while (changed) {
        ...
        const retainedHasUnknown = c.protectMultiArch && decisions.some((d) => d.disposition === "protected" && evidence.unknown.has(d.digest));
        for (const d of decisions) {
            if (d.disposition !== "eligible")
                continue;
            let reason;
            let relatedTo;
            ...
            if (!reason && evidence.unknown.has(d.digest))
                reason = "PROTECTED_UNKNOWN_RELATION";
            if (!reason && retainedHasUnknown && d.tags.length === 0)
                reason = "PROTECTED_UNKNOWN_RELATION";
            if (reason) {
                d.disposition = "protected";
                d.reason = reason;
                if (relatedTo)
                    d.matchedRule = relatedTo;
                changed = true;
            }
        }
    }
...
export function assertBudget(c, p) {
    const n = p.counts.eligible;
    const pct = p.counts.scanned === 0 ? 0 : (n / p.counts.scanned) * 100;
    if (n > c.maxDeletions)
        throw new Error(`ABORTED_BUDGET_EXCEEDED: ${n} deletions > max-deletions ${c.maxDeletions}`);
