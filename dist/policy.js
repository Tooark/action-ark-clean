...
function subjectsOf(decision, evidence) {
 ...
 */
const WEAK_RETENTIONS = ["PROTECTED_UNMATCHED_TAG", "PROTECTED_TOO_RECENT"];
 ...
 */
export function buildPlan(config, versions, now = new Date()) {
    const decisions = [];
 ...
 */
export function protectOciRelations(plan, evidence, config) {
    ...
    while (changed) {
        ...
        const retainedHasUnknown = config.protectMultiArch && decisions.some((d) => d.disposition === "protected" && evidence.unknown.has(d.digest));
 ...
 */
export function unresolvedSubjects(plan, evidence) {
 ...
 */
export function releaseOrphanReferrers(plan, evidence, confirmedAbsent, config) {
    ...
    const decisions = plan.decisions.map((d) => {
        ...
        return { ...d, disposition: "eligible", reason: "ELIGIBLE_ORPHAN_REFERRER", matchedRule: first };
    });
 ...
 */
export function capToBudget(config, plan) {
    const allowed = Math.max(0, Math.min(config.maxDeletions, Math.floor((config.maxDeletePercentage / 100) * plan.counts.scanned)));
    ...
    const decisions = plan.decisions.map((d) => d.disposition === "eligible" && !kept.has(d.versionId)
        ? { ...d, disposition: "protected", reason: "DEFERRED_BUDGET" }
        : { ...d });
    return {
        ...plan,
        decisions,
 ...
 */
export function assertBudget(config, plan) {
    ...
    if (pct > config.maxDeletePercentage) {
        throw new Error(`ABORTED_BUDGET_EXCEEDED: ${pct.toFixed(1)}% > max-delete-percentage ${config.maxDeletePercentage}%`);
    }
}
