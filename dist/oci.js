...
export async function gatherOciEvidence(c, versions) {
    const evidence = { children: new Map(), subjects: new Map(), unknown: new Set(), sizes: new Map() };
    let token;
    try {
 ...
 */
export async function confirmAbsent(c, digests) {
    ...
    await pool(list, c.concurrency, async (digest) => {
        try {
            const r = await fetchRetry(`${registry()}/v2/${repository(c)}/manifests/${encodeURIComponent(digest)}`, { headers: { Authorization: `Bearer ${token}`, Accept: MANIFEST_ACCEPT } }, c.retryCount);
            if (r.status === 404) {
                absent.add(digest);
            }
        }
