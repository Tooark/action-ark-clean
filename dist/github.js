...
function base(c) {
    const owner = c.ownerType === "organization" ? `orgs/${encodeURIComponent(c.owner)}` : `users/${encodeURIComponent(c.owner)}`;
...
async function request(c, url, init = {}, attempt = 0) {
...
export async function resolveOwnerType(c) {
    if (c.ownerType !== "auto")
        return c.ownerType;
...
export async function listVersions(c) {
    const all = [];
    const seen = new Set();
    let url = `${base(c)}?per_page=100`;
...
export async function deleteVersion(c, id) {
