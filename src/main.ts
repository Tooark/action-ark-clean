...
import {
  ...
  planHash,
  protectOciRelations,
  releaseOrphanReferrers,
  unresolvedSubjects,
} from "./policy.js";
import type { ApplyReport, ApplyResult, OciEvidence, ResolvedConfig } from "./types.js";
...
export async function run(): Promise<void> {
  ...
  const needsOci =
    ((c.protectMultiArch || c.protectReferrers) && plan.counts.eligible > 0) || c.deleteOrphanedReferrers;
  let evidence: OciEvidence | undefined;
  if (needsOci) {
