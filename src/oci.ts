...
async function fetchRetry(url: string, init: RequestInit, retryCount: number, attempt = 0): Promise<Response> {
...
function repository(c: Config): string {
...
async function registryToken(c: Config): Promise<string> {
  ...
  if (!r.ok) throw new Error(`Registry token exchange failed with HTTP ${r.status}`);
  const data = (await r.json()) as { token?: string };
  if (!data.token) throw new Error("Registry token exchange returned no token");
  return data.token;
}
...
export async function gatherOciEvidence(c: Config, versions: PackageVersion[]): Promise<OciEvidence> {
  ...
  try {
    ...
    for (const v of versions) evidence.unknown.add(v.name);
    return evidence;
  }
  ...
  await pool(versions, c.concurrency, async (v) => {
    try {
      ...
      if (Array.isArray(manifest.manifests))
        evidence.children.set(
          v.name,
          manifest.manifests.map((m) => m.digest).filter((d): d is string => typeof d === "string"),
        );
      if (manifest.subject?.digest) evidence.subjects.set(v.name, manifest.subject.digest);
    } catch {
      evidence.unknown.add(v.name);
    }
  });
