...
// github.js reads GITHUB_API_URL at module load. Each test starts its own mock
// server, so we re-import the module with a cache-busting query string to get a
// fresh instance bound to the current GITHUB_API_URL.
let counter = 0;
async function loadGithub(apiUrl) {
  process.env.GITHUB_API_URL = apiUrl;
  return import(`../dist/github.js?test=${counter++}`);
}
...
test("listVersions follows pagination across pages", async () => {
  ...
  try {
    const { listVersions } = await loadGithub(mock.url);
...
test("resolveOwnerType maps API owner types and honors explicit values", async () => {
  ...
  try {
    const { resolveOwnerType } = await loadGithub(mock.url);
...
test("deleteVersion maps 204, 404, and other statuses", async () => {
  ...
  try {
    const { deleteVersion } = await loadGithub(mock.url);
